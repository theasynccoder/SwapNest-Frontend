...
import { Plus, Package, MessageCircle, Heart, Settings, LogOut, CreditCard, Eye } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const fetchSubscription = async () => {
    const { data } = await supabase
      ...
      .select('listings_remaining')
      .eq('user_id', user?.id)
      .eq('payment_verified', true);
    if (data) {
      const total = data.reduce((sum: number, sub: UserSubscription) => sum + sub.listings_remaining, 0);
      setListingsRemaining(total);
    }
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-gradient-dark rounded-2xl p-8 mb-8 text-primary-foreground">
          ...
          <p className="text-primary-foreground/70">Manage your listings and connect with buyers</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-card rounded-xl p-5 shadow-sm border border-border">
            ...
            <p className="text-2xl font-bold">{listings.reduce((sum, p) => sum + p.views, 0)}</p>
          ...
          <Link to="/sell" className="block">
            <div className="bg-gradient-primary rounded-xl p-6 text-primary-foreground hover:shadow-glow transition-shadow">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-display text-xl font-semibold">Sell Something</h3>
                  <p className="text-primary-foreground/70 text-sm">List your item in minutes</p>
                </div>
              </div>
            </div>
          </Link>
          <Link to="/subscription" className="block">
            <div className="bg-card border-2 border-primary rounded-xl p-6 hover:shadow-lg transition-shadow">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-display text-xl font-semibold">Get More Listings</h3>
                  <p className="text-muted-foreground text-sm">Upgrade your plan today</p>
                </div>
              </div>
            </div>
          </Link>
              ...
              {listings.map((product) => (
                <div key={product.id} className="flex items-center gap-4 p-4 bg-muted/50 rounded-lg">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-primary font-semibold">₹{product.price_inr.toLocaleString()}</p>
                  </div>
                  <div className="text-right">
                    <span className={`inline-block px-2 py-1 rounded text-xs font-medium ${
                      product.status === 'active' ? 'bg-success/10 text-success' :
                      product.status === 'sold' ? 'bg-muted text-muted-foreground' :
                      'bg-warning/10 text-warning'
                    }`}>
                      {product.status}
                    </span>
                    <p className="text-sm text-muted-foreground mt-1">{product.views} views</p>
                  </div>
                </div>
              ))}
            </div>
